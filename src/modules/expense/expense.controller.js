...
import { createExpense, deleteExpense, getExpensesByUser } from "./expense.service.js";
...
expenseRouter.delete("/expense/:id", authMiddleware, async (req, res) => {
  try {
    const response = await deleteExpense(req.user.id, req.params.id);
    return res.send(response.data).status(response.statusCode);
  } catch (error) {
    console.log("err", error);
    if (error?.statusCode === 400)
