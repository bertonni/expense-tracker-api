...
import { createUser, getUserDataById } from "./user.service.js";
...
userRouter.get("/users/:id", authMiddleware, async (req, res) => {
  try {
    const response = await getUserDataById(req.params.id);
    return res.send({ ...response.data }).status(response.statusCode);
  } catch (error) {
    if (error?.statusCode === 400)
      res.send({ message: error.message }).status(error.statusCode);
